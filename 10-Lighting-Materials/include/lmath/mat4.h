...
static inline mat4 mat4_translate_in_place(mat4 m, float x, float y, float z) {
    vec4 t = {x, y, z, 0};
    int i;
    for (i = 0; i < 4; ++i) {
        vec4 r = mat4_row(m, i);
        m.raw[3*i] += vec4_mul_inner(r, t);
    }
    return m;
}
...
static inline mat4 mat4_look_at(vec3 eye, vec3 center, vec3 up) {
    ...
    vec3 u = vec3_norm(up);
    vec3 s = vec3_norm(vec3_mul_cross(f, u));
    u = vec3_mul_cross(s, f);
